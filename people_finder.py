...
"""
people_finder is the testing application for the model we used to gather our data ( the coco ssd ), 

its basic use is similair to friends finder however it is pre made to work with then included coco ssd 

if you would like to use a model other than the coco ssd use the model flag
"""
...
def main():
	...
	THRESHOLD = 0.9	#seems to give best results between varying lighting conditions
	...
	with detection_graph.as_default():
		with tf.Session(graph=detection_graph) as sess:
			...
			window = cv2.namedWindow("People-Finder", cv2.WINDOW_NORMAL)
			cv2.resizeWindow("People-Finder", int(SCREEN_WIDTH/3.2), int(SCREEN_HEIGHT/3.6) ) # These values maintain the aspect ratio I used for testing regardless of screen size
			while True:
				# Actual detection
				try:
					...
					cv2.imshow("People-Finder", image)
					cv2.waitKey(1)
